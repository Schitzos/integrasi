...
class PNSController extends Controller {
    ...
    public function index()
    {
        ...
        return view('pns.index')->with('data',$data);
    }
    ...
    public function create()
    {
        ...
        $rekeknig = DB::table('rekening')->where('kd_rekening','=',2)->get();
        return view('pns.create')->with('kegiatan',$kegiatan)->with('rekeknig',$rekeknig);
    }
    ...
    public function store()
    {
        ...
        if ($validator->fails()){   
            return Redirect::to('/pns/create')->withErrors($validator)->withInput();
        }else{
            $JmlPegawai = input::get('JmlPegawai');
            if (date('m', strtotime(Input::get('Tgl_PNS')))==1) {
                $bulan = 'Januari';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==2) {
                $bulan = 'Februari';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==3) {
                $bulan = 'Maret';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==4) {
                $bulan = 'April';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==5) {
                $bulan = 'Mei';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==6) {
                $bulan = 'Juni';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==7) {
                $bulan = 'Juli';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==8) {
                $bulan = 'Agustus';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==9) {
                $bulan = 'September';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==10) {
                $bulan = 'Oktober';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==11) {
                $bulan = 'November';
            } else if (date('m', strtotime(Input::get('Tgl_PNS')))==12) {
                $bulan = 'Desember';
            }
            ...
            array( 
                ...
                'tgl_cetak_honor'   => date('Y-m-d', strtotime(Input::get('Tgl_PNS'))),
            ...
            Session::flash('message', 'Data Honor Non PNS berhasil ditambahkan');
            return Redirect::to('/pns');
        }
    }
    public function edit($id)
    {
        ...
        $rekening = DB::table('rekening')->where('kd_rekening','=',2)->get();
        return view('pns.edit')->with('data',$data)->with('pegawai',$pegawai)->with('kegiatan',$kegiatan)
    ...
    public function update()
    {
        ...
        $validator = Validator::make(Input::all(), $rules,$messages);
        if ($validator->fails()) {   
            return Redirect::to('/pns/'.$id.'/edit')->withErrors($validator)->withInput();
        } else { 
            ...
            Session::flash('message', 'Data Honor Non PNS berhasil diubah');
            return Redirect::to('/pns');
        }       
    }
    public function cetak($id)
    {
        ...
        $ppk =  DB::table('pegawai as p')
            ...
            ->first();
        $view = View::make('pns.cetak', array('data' => $data, 'detail' => $detail, 'bendahara' => $bendahara, 'ppk' => $ppk, 'i' => 0))->render(); 
    ...
    public function destroy($id)
    {
        ...
        Session::flash('message', 'Data Honor Non PNS berhasil dihapus !');
        return Redirect::to('/pns');
    }
