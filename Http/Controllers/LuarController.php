...
class LuarController extends Controller {
    ...
    public function index()
    {
        ...
        return view('luar.index')->with('data',$data)->with('bulan',$bulan);
    }
    ...
    public function cetak($id)
    {
        $data =DB::table('perjalanan as p')
            ...
            ->select('p.*','g.*','k.*','p1.nip_pegawai as nip_tertugas','p1.nama_pegawai as nama_tertugas','p1.nip_pegawai as nip_pptk',
        ...
        $menugaskan = DB::table('pengikut_perjalanan as pp')
            ...
            ->where('pp.kode_perjalanan','=',$id)
            ->select('pg.*','pp.*','g.nama_golongan','j.nama_jabatan','b.nama_bidang','s.nama_seksi','pangkat','nama_golongan')
            ->orderby('g.id_golongan')
            ->get();
        ...
        $rincian = DB::table('pengikut_perjalanan as pp')
            ...
            ->get();
        $akomodasi = DB::table('akomodasi')->where('id_perjalanan','=',$data->id_perjalanan)
            ->get();
        ...
        foreach ($akomodasi as $ako) {
            $nilai+=$ako->jumlah*$ako->Hari*$ako->harga;
        }
        $bilang = $this->terbilang($nilai);
        $nom = explode('/', $data->nomor_perjalanan);
        $nomor = $nom[1];
        $tahun = $nom[3];
        $view = View::make('luar.cetak', array('data' => $data, 'pengikut' => $pengikut, 'ttd1' => $ttd1, 'ttd2' => $ttd2, 'nomor' => $nomor, 
            'tahun' => $tahun, 'bilang' => $bilang, 'nilai' => $nilai, 'rincian' => $rincian, 'ppk' => $ppk, 'bendahara' => $bendahara, 'hasil' => $hasil, 'i' => 0,'menugaskan'=> $menugaskan,'akomodasi'=> $akomodasi))->render(); 
    ...
    public function create()
    {
        ...
        return view('luar.create')->with('pegawai',$pegawai)->with('kegiatan',$kegiatan)->with('lokasi',$lokasi)->with('ttd',$ttd);
    }
    ...
    public function store()
    {
        ...
        if ($validator->fails()){   
            return Redirect::to('/luar')->withErrors($validator)->withInput();
        }
        else{
            ...
            $jmlAkomodasi = Input::get('jmlAko');
            for ($i=1; $i <= $jmlAkomodasi ; $i++) { 
                DB::table('akomodasi')->insert(
                array( 
                    'jumlah'              => Input::get('akojumlah'.$i),
                    'harga'               => Input::get('hargaAko'.$i),
                    'hari'                => Input::get('akohari'.$i),
            ...
            for ($i=0; $i < count($hasil)-1; $i++) { 
                ...
                array( 
                    'id_perjalanan'             => $perjalananlast->id_perjalanan,
                    'uraian_hasil_perjalanan'   => substr($hasil[$i],1)
                ));
            }
        ...
        if (Auth::user()->admin==1) {
            ...
            if(Auth::user()->pptk==1){
                ...
                $kegiatan = DB::table('kegiatan as k')
                    ->join('seksi  as s','k.id_seksi','=','s.id_seksi')
                    ->join('bidang  as b','s.id_bidang','=','b.id_bidang')
                    ->where('b.id_bidang','=',Auth::user()->id_bidang)->get();
            }
        }
        $akomodasi = DB::table('akomodasi')->where('id_perjalanan',$id)->get();
        $ttd = DB::table('pegawai')->whereIn('id_jabatan',[2,1])->get();
        $lokasi = DB::table('desa')->get();
        $hasil =  DB::table('hasil_perjalanan')->where('id_perjalanan','=',$id)->get();
        $jmlako = count($akomodasi);
        return view('luar.edit')->with('data',$data)->with('pengikut',$pengikut)->with('pegawai',$pegawai)->with('kegiatan',$kegiatan)->with('lokasi',$lokasi)->with('hasil',$hasil)->with('ttd',$ttd)->with('akomodasi',$akomodasi)->with('jmlako',$jmlako);
    }
    ...
    public function update()
    {
        ...
        $validator = Validator::make(Input::all(), $rules,$messages);
        if ($validator->fails()) {   
            return Redirect::to('/luar/'.$id.'/edit')->withErrors($validator)->withInput();
        } else { 
            $tgl= explode(" s/d ",Input::get('Tgl_Perjalanan'));
            $tglmulai = $tgl[0];
            $tglselesai = $tgl[1];
            ...
            for ($i=1; $i <= Input::get('jmlAko') ; $i++) { 
                DB::table('akomodasi')->where('id_akomodasi',Input::get('idako'.$i))->update(
                array( 
                    'jumlah'    => Input::get('akojumlah'.$i),
                    'harga'     => Input::get('hargaAko'.$i),
                    'Hari'      => Input::get('akohari'.$i)
    ...
    public function destroy($id)
    {
        ...
        DB::table('akomodasi')->where('id_perjalanan', '=',$id)->delete();
