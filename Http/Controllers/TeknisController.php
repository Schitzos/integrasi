...
class TeknisController extends Controller {
    ...
    public function index()
    {
        ...
        return view('teknis.index')->with('data',$data);
    }
    ...
    public function create()
    {
        ...
        $rekeknig = DB::table('rekening')->where('kd_rekening','=',1)->get();
        return view('teknis.create')->with('kegiatan',$kegiatan)->with('rekeknig',$rekeknig);
    }
    ...
    public function store()
    {
        ...
        if ($validator->fails()){   
            return Redirect::to('/teknis/create')->withErrors($validator)->withInput();
        }else{
            $JmlPegawai = input::get('JmlPegawai');
            if (date('m', strtotime(Input::get('Tgl_Teknis')))==1) {
                $bulan = 'Januari';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==2) {
                $bulan = 'Februari';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==3) {
                $bulan = 'Maret';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==4) {
                $bulan = 'April';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==5) {
                $bulan = 'Mei';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==6) {
                $bulan = 'Juni';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==7) {
                $bulan = 'Juli';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==8) {
                $bulan = 'Agustus';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==9) {
                $bulan = 'September';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==10) {
                $bulan = 'Oktober';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==11) {
                $bulan = 'November';
            } else if (date('m', strtotime(Input::get('Tgl_Teknis')))==12) {
                $bulan = 'Desember';
            }
            ...
            array( 
                ...
                'tgl_cetak_honor'   => date('Y-m-d', strtotime(Input::get('Tgl_Teknis'))),
            ...
            Session::flash('message', 'Data Honor Pelaksana Teknis berhasil ditambahkan');
            return Redirect::to('/teknis');
        }
    }
    public function edit($id)
    {
        ...
        $rekening = DB::table('rekening')->where('kd_rekening','=',1)->get();
        return view('teknis.edit')->with('data',$data)->with('pegawai',$pegawai)->with('kegiatan',$kegiatan)
    ...
    public function update()
    {
        ...
        $validator = Validator::make(Input::all(), $rules,$messages);
        if ($validator->fails()) {   
            return Redirect::to('/teknis/'.$id.'/edit')->withErrors($validator)->withInput();
        } else { 
            ...
            Session::flash('message', 'Data Honor Pelaksana Teknis berhasil diubah');
            return Redirect::to('/teknis');
        }       
    }
    public function cetak($id)
    {
        ...
        $ppk =  DB::table('pegawai as p')
            ...
            ->first();
        $view = View::make('teknis.cetak', array('data' => $data, 'detail' => $detail, 'bendahara' => $bendahara, 'ppk' => $ppk, 'i' => 0))->render(); 
    ...
    public function destroy($id)
    {
        ...
        Session::flash('message', 'Data Honor Pelaksana Teknis berhasil dihapus !');
        return Redirect::to('/teknis');
    }
